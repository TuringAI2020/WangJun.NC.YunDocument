    ...
    public class SyncTaskRunner
    {
        ...
        protected void AutoSync()
        {
            while (true)
            {
                ...
                //try
                //{
                //    var taskName = "增量同步关键词";
                //    var resKeys = REDIS.Current.QueryKeys("Stock:Sync:2DB:AI:Keywords:ALL");
                //    if (resKeys.SUCCESS)
                //    {
                //        var keys = resKeys.DATA as List<string>;
                //        if (null != keys && 0 < keys.Count)
                ...
                //                        keys.ForEach(p =>
                //                        {
                //                            this.IncSync<Keywords>(p, (w) => { return new object[] { w.Id }; });
                //                        });
                //                    }
                //                    catch (Exception ex)
                //                    {
                ...
                this.ExecuteIncSyncTask<Keywords>("增量同步关键词", "Stock:Sync:2DB:AI:Keywords:ALL", (w) => { return new object[] { w.Id }; });
                #endregion
